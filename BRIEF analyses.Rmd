...
<ul>
  <li>**`110`r nrow(subset(BRIEF, BRIEF$AgeMonths <= 71))` participants**</li>
</ul>  
```{r}
...
<ul>
  ...
  <li>Create numerical "AoAE" variable from Age of Auditory Exposure variable, converting "None" to current age</li>
  <li>Change structure of Age of Auditory Exposure from character to integer</li>
  <li>Remove participants for whom either of these two variables is missing
  <li>This results in **`r nrow(subset(BRIEF, BRIEF$AoAE!='' & BRIEF$Age.of.Language.Exposure..mo.!=''))` participants**</li>
</ul>  
```{r}
...
#Both Early groups significantly different from both Later groups.  Later ASL also significantly different from Later English (Later ASL has even ) 
```
